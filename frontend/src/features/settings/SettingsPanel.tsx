...
import { AudioLines, Bot, Database, Download, HardDrive, Loader2, RefreshCw, Save, ShieldCheck, Square, UploadCloud, Users, Volume2 } from "lucide-react"

import type { VendorConfigInput } from "@/api/agent"
import {
  synthesizeVoiceRolePreview,
  voiceRoleAvatarURL,
...
export default function SettingsPanel() {
  ...
  // 试听：同一时刻只允许一个角色在响，所以状态是「哪个角色」而不是「是否在响」。
  const [previewRoleId, setPreviewRoleId] = useState<string | null>(null)
  const [previewPhase, setPreviewPhase] = useState<"loading" | "playing">("loading")
  const [previewError, setPreviewError] = useState<string | null>(null)
  const previewAudioRef = useRef<HTMLAudioElement | null>(null)
  const previewURLRef = useRef<string | null>(null)

  // 停止播放、切换角色、组件卸载都走这里。只 pause 不 revoke 的话，blob 会一直
  // 挂到整页的生命周期上，试听几次就攒下一堆再也用不到的音频。
  ...
  async function togglePreview(role: VoiceRole) {
    // 再点一次就是停止；点别的角色则先掐掉当前这个，避免两个声音叠在一起。
    if (previewRoleId === role.id) {
      stopPreview()
      return
    }
    stopPreview()
    // 简介是这个角色的「名片」，所以试听读它；没写简介就退回读名字，
    // 总比按下去一声不响强。
    const text = role.bio?.trim() || role.name.trim()
    if (!text) return
    ...
    try {
      ...
      stopPreview()
      setPreviewError(cause instanceof Error ? cause.message : "试听失败")
    }
  }
      ...
      <div className="grid content-start gap-4">
        ...
        <Card>
          ...
          <CardContent className="grid gap-3">
                    ...
                    <div key={role.id} className={role.id === activeVoiceRoleId ? "grid gap-2 rounded-lg border border-primary/30 bg-primary/4 px-2.5 py-2" : "grid gap-2 rounded-lg border border-border bg-background/70 px-2.5 py-2"}>
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <div className="grid min-w-0 flex-1 gap-0.5">
                          ...
                          {/* 卡片只占一列，简介放不下就会被截断，所以挂个 title 兜住全文。 */}
                          <span className="truncate text-xs text-muted-foreground" title={role.bio?.trim() || undefined}>{role.bio?.trim() ? role.bio : "尚无简介"}</span>
                        </div>
                        <div className="flex shrink-0 flex-wrap items-center gap-1.5">
                          <Button
                            size="xs"
                            variant="ghost"
                            aria-label={previewRoleId === role.id ? `停止试听 ${role.name}` : `试听 ${role.name}`}
                            title={role.bio?.trim() ? "试听这句简介" : "试听（该角色还没写简介，读名字）"}
                            onClick={() => void togglePreview(role)}
                          >
            ...
            {previewError ? <p role="alert" className="rounded-lg border border-destructive/35 bg-destructive/5 px-3 py-2 text-xs text-destructive">{previewError}</p> : null}
